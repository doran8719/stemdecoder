...
def inject_global_styles():
    st.markdown(
        ...
        -------------------------------------------------- */
        .stApp {
            background: radial-gradient(circle at top left, #ffffff 0, #f5f5f8 35%, #ececf3 100%);
            color: #111827;
        ...
        /* Make normal body text nicely readable */
        .block-container,
        .block-container p,
        ...
        .bd-header {
            ...
            padding: 18px 22px;
            border-radius: 22px;
            background: linear-gradient(135deg, #fdfdfd, #f1f1f8);
            box-shadow:
        ...
        /* --------------------------------------------------
           TABS (Process / My Songs / Settings)
        -------------------------------------------------- */
        ...
        -------------------------------------------------- */
        .bd-section {
            background: linear-gradient(145deg, #ffffff, #f3f3fb);
            border-radius: 18px;
        ...
        /* Song/detail cards inside sections */
        .bd-card {
            position: relative;
            background: #ffffff;
            border-radius: 16px;
        ...
        .song-card .stButton > button {
            width: 100%;
            aspect-ratio: 1 / 1;              /* each tile a square */
            border-radius: 18px;
            border: 1px solid #e5e5ea;
            background: #ffffff;
    ...
    st.markdown(
        ...
        <div class="bd-header">
          <div class="bd-header-left">
            <div class="bd-title">BeatDecoder</div>
            <div class="bd-subtitle">
              AI stem splitter · MIDI rebuilder · chord & section analyzer
            </div>
          </div>
          <div class="bd-header-right">
            <span class="bd-pill-badge">BETA</span>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
st.set_page_config(
    page_title="BeatDecoder",
    page_icon="🎚️",
    layout="wide",
)

# ===== Global CSS Theme (LALAL-style, premium look) =====
st.markdown(
    """
    <style>
    ...
    /* Page background */
    .stApp {
        background: radial-gradient(circle at top, #101827 0, #020617 45%, #000000 100%);
        color: #e5e7eb;
    ...
    .bd-hero-pill {
        ...
        padding: 3px 10px;
        border-radius: 999px;
        background: radial-gradient(circle at top left, #22c55e33, #22c55e05);
    ...
    button[aria-selected="true"][role="tab"] {
        background: radial-gradient(circle at top, #22c55e33, #22c55e0a) !important;
    ...
    .bd-primary-btn > button {
        ...
        box-shadow:
            0 14px 35px rgba(22,163,74,0.35),
            0 0 0 1px rgba(21,128,61,0.7);
    }
    .bd-primary-btn > button:hover {
    ...
    /* Audio players & download buttons spacing */
    .bd-stem-row {
        padding: 6px 0 10px 0;
        border-bottom: 1px solid rgba(31,41,55,0.8);
        margin-bottom: 6px;
    }
    .bd-stem-row:last-child {
        border-bottom: none;
        margin-bottom: 0;
        padding-bottom: 0;
    }

    .bd-download-btn > button {
        border-radius: 999px !important;
        font-size: 11px !important;
...
with tab_process:
    ...
    # ---------------- LEFT: Upload + Controls Card ----------------
    with col_left:
        st.markdown('<div class="bd-section">', unsafe_allow_html=True)

        # Section header (title + small caption)
        st.markdown(
            """
            <div class="bd-section-header">
                <div class="bd-section-title">Process track(s)</div>
                <div class="bd-section-caption">
                    Upload a song and decode it into stems, MIDI & analysis.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        # Backend badge (Remote GPU vs Local)
        backend_label = st.session_state.get("processing_backend", "Local (this server)")
        st.markdown(
            f"<div class='bd-badge'>Backend: {backend_label}</div>",
            unsafe_allow_html=True,
        )

        st.caption("Upload one or multiple WAV/MP3 files to process with AI.")
    ...
    # ---------------- RIGHT: Log Panel Card ----------------
    with col_right:
        st.markdown('<div class="bd-section bd-log-box">', unsafe_allow_html=True)
        st.markdown(
            """
            <div class="bd-section-header">
                <div class="bd-section-title">Processing log</div>
                <div class="bd-section-caption">
                    High-level status messages for this session.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
            ...
            with col_left:
                ...
                if not worker_url:
                    ...
                    try:
                        ...
                        st.success("Your track is now processing on the GPU.")
                        ...
                        with st.spinner("Processing on GPU worker..."):
                            ...
                            for _ in range(max_polls):
                                ...
                                # We still read status internally, but don’t show logs anymore
